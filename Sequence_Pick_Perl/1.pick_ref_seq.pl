...
while (<IN1>) {
	...
	#Find the line which starts with "Scores for complete sequences".
}

$n=0;	
#count the number of the suitable resultes.
@Id_no="";
while (<IN1>) {  
	#use the  "while" cycle to find the suitable lines.
	chomp ($_);  
	#cut down the sign of"\n".
	if (index($_,"ENS")!=-1) {
		# the index function is used to find lines containing the string of "ENSP".
		@data=split(/\s+/,$_);   
		#format the line into an array.
		my $id=$data[9]; my $score=$data[2];my $evalue=$data[1]; 
		#assign the variable according to the index of the array.
...
print OUT2 ">";
while (<IN2>) {
	#search for sequences with the selected id
for ($i=0;$i<$n ;$i++) {
